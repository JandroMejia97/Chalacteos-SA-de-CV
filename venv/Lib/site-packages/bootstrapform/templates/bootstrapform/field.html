...
<div class="form-group{% if field.errors %} has-error{% endif %}">
    {% if field|is_checkbox %}
        <div class="{{ classes.single_value }}">
            <div class="checkbox">
                {% if field.help_text %}
                    <p class="help-block alert alert-info small text-muted">
                        {{ field.help_text|safe }}
                    </p>
                {% endif %}
                {% if field.auto_id %}
                    <strong><span class="control-label bold">{{ field.label }}</span></strong>
                    <label {% if field.field.required and form.required_css_class %}class="{{ form.required_css_class }} bold"{% endif %}>
                        {{ field }} 
                    </label>
                {% endif %}
                {% for error in field.errors %}
                    <div class="help-block alert alert-danger small text-muted in alert-dismissible">
